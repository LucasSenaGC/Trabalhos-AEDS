// -------------------------------------------------------------------------------- //
//GABARITO Utilizado para resolver a questa 08TP02 foi desenvolvido por:@author Pedro Henrique Lopes Costa
//Modificacoes e implementacao da lista feitas por: @author Lucas Sena Guimaraes de Castro
// 4/2022
//
// -------------------------------------------------------------------------------- //
// Includes
...
Lista makeLista(){
    Lista listas;
    listas.Movies = malloc (300 * sizeof(Movie));
    listas.n = 0;
}

//INSERIR INICIO
void inserirInicio(Lista *listas, Movie Movie, int tamanho) {
    ...
    for(int i = listas->n; i > 0; i--){
        listas->Movies[i] = listas->Movies[i-1];
    }

    listas->Movies[0] = Movie;
    listas->n++;
}

//INSERIR FIM
void inserirFim(Lista *listas, Movie Movie, int tamanho){
    ...
    listas->Movies[listas->n] = Movie;
    listas->n++;
}

//INSERIR POSICAO
void inserir(Lista *listas, Movie Movie, int tamanho, int pos) {
    ...
    listas->Movies[pos] = Movie;
    listas->n++;
}

//REMOVER INICIO
Movie removerInicio(Lista *listas) {

    //validar remocao
    if (listas->n == 0) {
        printf("Erro ao remover!");
    }
    Movie resp = listas->Movies[0];
    //printf("(R) ");
    //printf("%s\n", resp.nome);
...
//REMOVER FIM
Movie removerFim(Lista *listas){
    
    //validar remocao
    if (listas->n == 0) {
        printf("Erro ao remover!");
    }
    Movie resp = listas->Movies[listas->n];
    //printf("(R) ");
    //printf("%s\n", resp.nome);

    return listas->Movies[--listas->n ];
}
//REMOVER POSICAO
Movie remover(Lista *listas, int pos) {
    //validar remocao
    if (listas->n == 0 || pos < 0 || pos >= listas->n){
        printf("Erro ao remover!");
    }
    Movie resp = listas->Movies[pos];
    //printf("(R) ");
    //printf("%s\n", resp.nome);
    listas->n--;
...
//                    END LIST
//--------------------XXXXXXXXX--------------------------
//le ate encontrar espaco
char *lerAteEspaco(char string[]){
...
int main() {
    ...
    int n = 0;
    Lista lista = makeLista();
    makeListaNzero(&lista);
    readline(line + prefix_size, MAX_LINE_SIZE);

    while(!isFim(line + prefix_size)) {
        movie_readHtml(line);
        inserirFim(&lista, movies[count_movies++], 300);
        readline(line + prefix_size, MAX_LINE_SIZE);
    }
    ...
    for(int i = 0; i < n+1; i++){
        ...
        if(line[12] == 'I' && line[13] == 'I'){
            ...
            inserirInicio(&lista, movies[count_movies++], 100);
        
        //-------------------------------I*----------------------------    
        }else if(line[12] == 'I' && line[13] == '*'){
            ...
            memset(aux, 0, MAX_LINE_SIZE);
            strcpy(aux, "/tmp/filmes/");
            count = strlen(FDR_PREFIX);
            for(int j = 0; j < strlen(line); j++){
                if(j != 12 && j != 13 && j != 14){
            ...
            inserir(&lista, movies[count_movies++], 100, pos);

        //------------------------------IF-----------------------------    
        }else if(line[12] == 'I' && line[13] == 'F'){
            ...
            movie_readHtml(aux);
            inserirFim(&lista, movies[count_movies++], 100);
        
        //----------------------------RI-------------------------------
        }else if(line[12] == 'R' && line[13] == 'I'){
            removido = removerInicio(&lista);
            count_movies--;
            printf("(R) %s\n", removido.name);

        //---------------------------R*--------------------------------
        }else if(line[12] == 'R' && line[13] == '*'){ 
