...
class Lista {
    ...
    */
    public void makeLog(long inicio, long fim){
        Arq.openWrite("matricula_ binaria.txt");
    ...
    //PESQUISA SEQUENCIAL
    public boolean pesqSeq(String nome){
        boolean resp = false;
        for(int i = 0; i < n; i++){
            comparacoes++;
           if(filmes[i].getNome().compareTo(nome) == 0){
              resp = true;
              i = n;
           }
        }
    ...
    /**
    * Algoritmo de ordenacao por selecao para o atributo TituloOriginal.
    */
   public void SelectionSort() {
      for (int i = 0; i < (n - 1); i++) {
         ...
         for (int j = (i + 1); j < n; j++){
            comparacoes++;
            if ((int)filmes[menor].getTituloOriginal().charAt(0) > (int)filmes[j].getTituloOriginal().charAt(0)){
               menor = j;
            }else if (filmes[menor].getTituloOriginal().charAt(0) == filmes[j].getTituloOriginal().charAt(0)){
                if((int)filmes[menor].getTituloOriginal().charAt(1) > (int)filmes[j].getTituloOriginal().charAt(1)){
                    menor = j;
                    comparacoes += 2;
                }else if((int)filmes[menor].getTituloOriginal().charAt(1) == (int)filmes[j].getTituloOriginal().charAt(1)){
                    if((int)filmes[menor].getTituloOriginal().charAt(2) > (int)filmes[j].getTituloOriginal().charAt(2)){
                        menor = j;
                        comparacoes += 2;
                    }else if((int)filmes[menor].getTituloOriginal().charAt(2) == (int)filmes[j].getTituloOriginal().charAt(2)){
                        if((int)filmes[menor].getTituloOriginal().charAt(3) > (int)filmes[j].getTituloOriginal().charAt(3)){
                            menor = j;
                            comparacoes += 2;
                        }else if((int)filmes[menor].getTituloOriginal().charAt(3) == (int)filmes[j].getTituloOriginal().charAt(3)){
                            if((int)filmes[menor].getTituloOriginal().charAt(4) > (int)filmes[j].getTituloOriginal().charAt(4)){
                                menor = j;
                                comparacoes += 2;
                            }else if((int)filmes[menor].getTituloOriginal().charAt(4) == (int)filmes[j].getTituloOriginal().charAt(4)){
                                if((int)filmes[menor].getTituloOriginal().charAt(5) > (int)filmes[j].getTituloOriginal().charAt(5)){
                                    menor = j;
                                    comparacoes += 2;
                                }
                            }
    ...
    //PESQUISA BINARIA (Temos que ordenar primeiro por ordem alfabetica para depois utilizar a pesquisa binaria por nomes);
    public boolean pesqBin(String nome){
        boolean resp = false;
        int dir = (n - 1), esq = 0, meio;
  
        while (esq <= dir){
            meio = (esq + dir) / 2;
            if(filmes[meio].getNome().compareTo(nome) == 0){
                comparacoes++;
                resp = true;
                esq = dir + 1;
            /*para comparar o array e necessario verificar as letras inicias e caso as mesmas sejam iguais, devemos desempatar
              obtendo a proxima letra*/
            }else if((int) nome.charAt(0) > (int)filmes[meio].getNome().charAt(0)) {
                comparacoes++;
                esq = meio + 1;
            }else if((int) nome.charAt(0) == (int)filmes[meio].getNome().charAt(0)){
                comparacoes++;
                if((int) nome.charAt(1) > (int)filmes[meio].getNome().charAt(1)){
                    esq = meio + 1;
                }else{
                    dir = meio - 1;
                }
