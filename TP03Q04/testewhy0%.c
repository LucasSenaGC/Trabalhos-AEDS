...
//                    FIM LISTA
//--------------------XXXXXXXXX--------------------------
void Registro(clock_t inicio, clock_t fim, Lista *lista){
    double tempo = 0;
    tempo = ((double)fim - inicio) / 1000.0;
    FILE *fp = fopen("matricula_shellsort.txt", "w");
    fprintf(fp, "735102\t%d\t%d\t%f", COMP, MOV, tempo);
    fclose(fp);
}
...
int main(){
    ...
    do{
        ...
        /*if(strstr(endereco, "Zack Snyder.html") != NULL){
            strcpy(endereco, "/tmp/filmes/Liga da Justica de Zack Snyder.html");
        }*/
