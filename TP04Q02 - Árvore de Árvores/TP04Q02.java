...
class ArvoreBinaria {
	...
	private boolean pesquisar(String x, No i) {
		...
		if (i == null) {
         resp = false;
      } else if (pesquisar2(x, i.segundo) == true) {
         resp = true;
         comparacoes++;
    ...
    */
    public void makeLog(long inicio, long fim){
        Arq.openWrite("matricula arvoreArvore.txt");
    ...
    public No inserir(char x, No i) throws Exception{
        if (i == null) {
            i = new No(x);
   
         } else if ((int)x < (int)i.elemento) {
	 ...
	 */
	public void inserir2(Filme x) throws Exception {
	 ...
	 * @throws Exception Se o elemento existir.
	 */
	private No inserir2(Filme x, No i) throws Exception {
		if (i == null) {
            throw new Exception("Erro ao inserir!");
      } else if ((int)x.getTituloOriginal().charAt(0) < (int)i.elemento) {
         ...
         i.segundo = inserir3(x, i.segundo);
      }
		return i;
	}

    private No2 inserir3(Filme x, No2 i) throws Exception{
        if (i == null) {
            i = new No2(x);
...
class TP04Q02{
    ...
    public static void main(String[] args) throws Exception{
        ...
        arvore.inserir('D');arvore.inserir('R');arvore.inserir('Z');arvore.inserir('X');
        arvore.inserir('V');arvore.inserir('B');arvore.inserir('F');arvore.inserir('P');
        arvore.inserir('U');arvore.inserir('I');arvore.inserir('G');arvore.inserir('E');
        arvore.inserir('J');arvore.inserir('L');arvore.inserir('H');arvore.inserir('T');
        arvore.inserir('A');arvore.inserir('W');arvore.inserir('S');arvore.inserir('O');
        arvore.inserir('M');arvore.inserir('N');arvore.inserir('K');arvore.inserir('C');
        arvore.inserir('Y');arvore.inserir('Q');
        for(int i = 0; i<count; i++){
            filmes[i] = new Filme();
            filmes[i].ler(entradas[i]);
            replica = filmes[i].clone();
            arvore.inserir2(replica);
        }
